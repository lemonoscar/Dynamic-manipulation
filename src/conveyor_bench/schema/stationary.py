...
NUREC_BACKEND = "isaac_rtx_native_nurec"
...
def _stationary_target_asset_id(episode: Mapping[str, Any]) -> str:
    ...
    if not isinstance(scene, Mapping) or scene.get("backend") != NUREC_BACKEND:
        return STATIONARY_TARGET_ASSET_ID
    fixture = _mapping(
    ...
    if (
        ...
        raise ValueError("stationary object fixture did not pass")
    objects = _sequence(fixture.get("objects"), "object fixtures")
    object_ids = [
        item.get("object_id")
        for item in objects
    ...
    if (
        ...
        raise ValueError("stationary profile requires one object fixture")
    return object_ids[0]
