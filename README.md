...
当时连续 PCT 末段为候选；本轮实际执行与失败归因见下方更新，门槛没有放宽。
见[新开发与实验文档](docs/execution_interfaces_v2_20260906.md)，其中区分接触未知、几何代理、执行完成和模型成绩。

**2026-09-06 新实验**：训练集 27,871 个 MANI chunk 中，2,022 个含失效缓存，11,819 个需因失效或来源未知建议隔离（未知不等于污染）。两套 A/B/C 共 18 个局部窗口中，000006 出现计划替换推迟闭合并降低抬升的证据，000030 的替换分支也能抬升；窗口不足以验收持续抓取。源 NAV 已实际执行，但 10 秒后位置误差 23.91 cm，未通过到达验收；定位到 DWA 15 cm 停止与外层 12 cm 到达要求不一致。
见[本轮结果与证据边界](docs/plan_provenance_fork_results_20260906.md)。

**2026-09-07 框架修正（待审核）**：独立实现任务级 rolling plan、动作队列/VJP RTC、分阶段专家与新数据训练入口。650 项回归通过；原权重采样兼容性通过；4 样本 CPU 开环的裁剪门仍失败。未启动新数据训练或物理最终验收。见[开发交付与验证记录](docs/vla_framework_revision_20260907.md)。
