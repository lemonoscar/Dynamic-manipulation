"""Shared single-owner Isaac Sim collection loop."""
...
from conveyor_bench.schema.assets import (
    ASSET_LOCK_PATH,
    ObjectAsset,
    ReceptacleAsset,
...
from conveyor_bench.schema.protocol import (
    CanonicalAction,
    EpisodeManifest,
    Event,
    EventKind,
...
from conveyor_bench.schema.recorder import EpisodeRecorder
...
class _RuntimeOptions:
    ...
    # Profile-owned object pools keep the canonical protocol default intact while
    # allowing the current scene to use sidecar assets without global mutation.
    ...
    def __post_init__(self) -> None:
        ...
        if self.device != "cpu":
            raise ValueError(
                "the current benchmark requires --device cpu: its validated "
...
class _ConveyorRuntimeCore:
    ...
    def _make_scene_cfg(self) -> ConveyorWorkcellCfg:
        """Build the scene configuration while preserving protocol defaults."""

        scene_cfg = ConveyorWorkcellCfg(
            num_envs=1,
            env_spacing=3.0,
    ...
    def _run_episode(
        ...
        *,
        ...
        if len(resolved.targets) > 1:
            from conveyor_bench.task_coordinator import (
    ...
    def _mobile_turn_forward_speed_mps(
        ...
        # validated mobile-delivery task.
        return 0.0
