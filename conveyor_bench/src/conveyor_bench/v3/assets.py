"""Validate the SSH-delivered V3 asset bundle and compose its runtime USD.
...
ASSET_ROOT_ENV = "CONVEYOR_BENCH_V3_ASSET_ROOT"
...
def resolve_asset_root(value: str | Path | None = None) -> Path:
    ...
    raw_value = value if value is not None else os.environ.get(ASSET_ROOT_ENV)
    if raw_value is None or not str(raw_value).strip():
        raise ValueError(
            f"V3 assets require --asset-root or {ASSET_ROOT_ENV}"
        )
    path = Path(raw_value).expanduser()
    if not path.is_absolute():
        raise ValueError("V3 asset root must be an absolute path")
    if path.is_symlink():
        raise ValueError("V3 asset root cannot be a symlink")
    resolved = path.resolve(strict=True)
    if not resolved.is_dir():
        raise NotADirectoryError(f"V3 asset root is not a directory: {resolved}")
    return resolved
...
def validate_asset_bundle(
    ...
    *,
    ...
    return V3AssetBundle(root=resolved_root, report=report)
