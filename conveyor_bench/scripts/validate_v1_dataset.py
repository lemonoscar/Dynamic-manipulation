#!/usr/bin/env python3
"""Validate published ConveyorBench V1 run artifacts."""
...
from conveyor_bench.v1.validation import validate_v1_dataset  # noqa: E402


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Validate a ConveyorBench V1 run summary or output root.",
    )
    parser.add_argument(
        "source",
        type=Path,
    ...
    print(
        f"V1 dataset invalid: {len(result.errors)} error(s)",
        file=sys.stderr,
    )
    return 1
