#!/usr/bin/env python3
"""Collect, gate, and index current PCT-scene demonstrations."""
    ...
    environment.update(
        {
            ...
            "PYTHONUNBUFFERED": "1",
            "CONVEYOR_BENCH_ASSET_ROOT": str(args.asset_root),
        }
    )
...
def _gate_episode(
    ...
    commands = [
        ...
        [str(args.python), "-B", str(SCRIPTS / "audit_episode.py"), str(episode)],
        [
            str(args.python),
            "-B",
            str(SCRIPTS / "check_camera_gate.py"),
            str(episode),
            "--output",
...
def _resolve(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if args.belt_speed not in {0.0, DYNAMIC_BELT_SPEED_MPS}:
        raise CollectionError(
            "collection belt speed must be 0.0 or 0.01 m/s"
        )
    ...
    if args.belt_speed == 0.0 and not requested_seeds <= STATIONARY_SEEDS:
        raise CollectionError(
            "stationary collection requires registered scenario seeds"
        )
