#!/usr/bin/env python3
"""Run the current ConveyorBench task in the Liangzhu NuRec scene."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "outputs" / "scene_smoke",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.enable_cameras:
        parser.error("native NuRec collection requires --enable_cameras")
    ...
    print(
        f"asset preflight passed: {bundle.report.file_count} files",
        flush=True,
    )
    ...
    print("Isaac application ready", flush=True)
    try:
        from conveyor_bench.cli import collection_exit_code
        from conveyor_bench.isaac.runtime import (
            RuntimeOptions,
            run_collection,
        )
        ...
        print("creating physical scene and collector", flush=True)
        try:
            ...
            print(
                f"collector raised {type(error).__name__}: {error!r}",
                file=sys.stderr,
                flush=True,
            )
            raise
