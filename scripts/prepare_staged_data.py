...
from conveyor_bench.conveyorvla.staged_data import RawEpisode,validate_family_splits,digest
...
def main(argv=None):
    ...
    episodes=[RawEpisode(path) for path in a.episodes]
    ...
    manifest={'schema':'staged-release-v2','time_profile':a.profile,'action_rows':len(actions),
        'task_rows':len(tasks),'quarantined_chunks':len(quarantine),'synthetic':any(e.manifest.get('synthetic',False) for e in episodes),
