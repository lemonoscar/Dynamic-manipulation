#!/usr/bin/env python3
"""Frozen full-release sampling and transition generation for full-episode RGB weights."""
...
def main():
    ...
    p.add_argument('--wall-seconds',type=int,default=2700)
    ...
    del saved
    protocol=dict(schema='full-episode-rgb-open-loop-v1',identity=identity,seeds=[17,29,43],
    ...
    for split in ('validation','test'):
        if split=='test':
            if source_files!={str(p.relative_to(a.repo)):sha(p) for p in (a.repo/'src/conveyor_bench/conveyorvla').rglob('*.py')}:raise ValueError('model/codec source changed before test')
            frozen=json.loads((a.output/'validation_report.json').read_text())
            if frozen['status']!='complete' or frozen['protocol']!=protocol:raise ValueError('test requires completed frozen validation')
        pool=[r for r in actions if r['split']==split];tr=[r for r in transitions if r['split']==split]
        pool.sort(key=lambda r:(r['task_family_id'],r['query_time_s']));rows=[];trs=[]
        ...
        report=dict(status='complete',split=split,full_split=True,protocol=protocol,metrics=aggregate(rows,trs),
